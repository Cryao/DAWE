...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    for (a = 0; a < layer1_size; ++a)
    	attention_syn[a] = 0;
    int window2 = 2;
    for (a = 0; a < window2 * 2 + 1; a++) if (a != window2) { //calc the average of context word embeddings
      ...
      if (negative > 0) for (d = 0; d < negative + 1; d++) {
        ...
        if (syn0[target].num == 1) {
          ...
          for (p = 0; p < syn0[target].num; ++p) {
            ...
            int mm, nn;
            real temp_total_sememe = 0;
            real min_exp_sememe = 10000000000000;
            int i = (p == 0 ? 0 : syn0[target].meaning_word_cnt[p - 1]);
            for (mm = i; mm < syn0[target].meaning_word_cnt[p]; ++mm) { // calc the weight for each sememe
              _exp_sememe[mm] = vectorDot(&meaning_syn[syn0[target].meaning_word_rank[mm] * layer1_size], attention_syn, layer1_size);
              if(_exp_sememe[mm] < min_exp_sememe)
                min_exp_sememe = _exp_sememe[mm];
            }
            for (mm = i; mm < syn0[target].meaning_word_cnt[p]; ++mm) {
              _exp_sememe[mm] = _exp_sememe[mm] - min_exp_sememe;
              if(_exp_sememe[mm] >= 3.999)
                _exp_sememe[mm] = 3.99;
              _exp_sememe[mm] = pre_exp[(int)(_exp_sememe[mm] * 250)];
              temp_total_sememe += _exp_sememe[mm];
            }
            for (mm = 0; mm < layer1_size; ++mm) avg_meaning[begin + mm] = 0;
            for (mm = i; mm < syn0[target].meaning_word_cnt[p]; ++mm) { // get "attention" result on sememes
              _exp_sememe[mm] = _exp_sememe[mm] / temp_total_sememe;
              real *temp = &meaning_syn[syn0[target].meaning_word_rank[mm] * layer1_size];
              for (nn = 0; nn < layer1_size; ++nn) {
                avg_meaning[begin + nn] += _exp_sememe[mm] * temp[nn];
              }
            }
        ...
        if (syn0[target].num == 1) {
          ...
          for (p = 0; p < syn0[target].num; ++p) {
            ...
            for (q = (p == 0 ? 0 : syn0[target].meaning_word_cnt[p - 1]); q < syn0[target].meaning_word_cnt[p]; ++q) {
              for (c = 0; c < layer1_size; ++c)
              {
                mult_part2[c] = mult_part[c] * (temp_mult[c] - attention[c]) * _exp[p] * _exp_sememe[q];
              }
...
void TrainModel() {
  ...
  // if (checkpoint[0] != 0) {
  //   fo = fopen("./vectors/SAT2_sense_vectors.bin", "w");
  //   for (a = 0; a < vocab_size; a++) {
  //     fprintf(fo, "%s ", vocab[a].word);
  //     if (syn0[a].num > 1) {
  ...
  //   fo = fopen("./vectors/SAT2_syn1neg_vectors.bin", "w");
  //   for (a = 0; a < vocab_size; a++) {
  //     fprintf(fo, "%s ", vocab[a].word);
