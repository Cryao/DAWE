...
real alpha = 0.000084, starting_alpha, sample = 1e-3;
...
void *TrainModelThread(void *id) {
  ...
  real *_exp_sememe = (real *)calloc(223, sizeof(real));
  real *arr = (real *)calloc(layer1_size * 50, sizeof(real));
  real *mult_part = (real *)calloc(layer1_size, sizeof(real));
  real *grad_syn1neg = (real *)calloc(layer1_size, sizeof(real));
  real total = 0;

  real *mult_sense_value = (real *)calloc(layer1_size * 50, sizeof(real));
  ...
  while (1) {
    ...
    for (a = b; a < window * 2 + 1 - b; a++) if (a != window) { //calc the sum of sememes for each sense of target word
      ...
      for (c = 0; c < syn0[last_word].num * layer1_size; ++c)
        mult_sense_value[c] = syn0[last_word].mult_sense_value[c];
      ...
      if (negative > 0) for (d = 0; d < negative + 1; d++) {
        ...
        if (syn0[last_word].num > 1) {
          for (p = 0; p < syn0[last_word].num; ++p) {
            ...
            int i = (p == 0 ? 0 : syn0[last_word].meaning_word_cnt[p - 1]);
            for (mm = i; mm < syn0[last_word].meaning_word_cnt[p]; ++mm) { // calc the weight for each sememe
              _exp_sememe[mm] = vectorDot(&meaning_syn[syn0[last_word].meaning_word_rank[mm] * layer1_size], &syn1neg[l2], layer1_size);
              if(_exp_sememe[mm] < min_exp)
                min_exp = _exp_sememe[mm];
            }
            for (mm = i; mm < syn0[last_word].meaning_word_cnt[p]; ++mm) {
              _exp_sememe[mm] = _exp_sememe[mm] - min_exp;
              if(_exp_sememe[mm] >= 3.999)
                _exp_sememe[mm] = 3.99;
              _exp_sememe[mm] = pre_exp[(int)(_exp_sememe[mm] * 250)];
              temp_total += _exp_sememe[mm];
            }
            for (mm = 0; mm < layer1_size; ++mm) arr[begin + mm] = 0;
            for (mm = i; mm < syn0[last_word].meaning_word_cnt[p]; ++mm) { // get "attention" result on sememes
              _exp_sememe[mm] = _exp_sememe[mm] / temp_total;
              real *temp = &meaning_syn[syn0[last_word].meaning_word_rank[mm] * layer1_size];
              for (nn = 0; nn < layer1_size; ++nn) {
                arr[begin + nn] += _exp_sememe[mm] * temp[nn];
              }
            }
          }
        }
          ...
          for (p = 0; p < layer1_size; ++p) {
            attention[p] = mult_sense_value[p];
          }
        }
        ...
        if (syn0[last_word].num == 1) {
          ...
          for (p = 0; p < syn0[last_word].num; ++p) { // get "attention" result on senses
            _exp[p] = _exp[p] / temp_total;
            for (q = 0; q < layer1_size; ++q) {
              attention[q] += _exp[p] * mult_sense_value[p * layer1_size + q];
            }
          }
          total = temp_total;
        }
        ...
        if (syn0[last_word].num == 1) {
          for (p = 0; p < layer1_size; ++p)
            syn0[last_word].mult_sense_value[p] += neu1e[p];
        }
        else {
        	for (p = 0; p < layer1_size; ++p)
        		grad_syn1neg[p] = 0.0;
          for (p = 0; p < syn0[last_word].num; ++p) {
            ...
            for (q = (p == 0 ? 0 : syn0[last_word].meaning_word_cnt[p - 1]); q < syn0[last_word].meaning_word_cnt[p]; ++q) {
              ...
              for (nn = 0; nn < layer1_size; ++nn) {
                mult_part[nn] = (mult_sense_value[p * layer1_size + nn] - attention[nn]) * _exp[p] * syn1neg[l2 + nn] * _exp_sememe[q] * neu1e[nn];
              }
          ...
          for (p = 0; p < syn0[last_word].num; ++p) {
            ...
            for (q = 0; q < layer1_size; ++q)
              temp[q] += neu1e[q] * _exp[p];
          }
        }

        for (c = 0; c < layer1_size; c++) {
          syn1neg[c + l2] += g * attention[c];
        }
      }
    }
    sentence_position++;
...
void TrainModel() {
  ...
  // if (checkpoint[0] != 0) {
  //   fo = fopen("./vectors/DAC2_sense_vectors.bin", "w");
  //   for (a = 0; a < vocab_size; a++) {
  //     fprintf(fo, "%s ", vocab[a].word);
  //     if (syn0[a].num > 1) {
	//     for (d = 1; d < syn0[a].num; ++d) {
	//         for (c = 0; c < layer1_size; ++c) {
	//             syn0[a].mult_sense_value[c] += syn0[a].mult_sense_value[d*layer1_size + c];
	//         }
	//     }
  //     }
  //     for (b = 0; b < layer1_size; b++) 
  ...
  //   fo = fopen("./vectors/DAC_syn1neg_vectors.bin", "w");
  //   for (a = 0; a < vocab_size; a++) {
  //     fprintf(fo, "%s ", vocab[a].word);
  //     for (b = a * layer1_size; b < (a+1) * layer1_size; b++) {
  //       fprintf(fo, "%lf ", syn1neg[b]);
  //     }
  //     fprintf(fo, "\n");
  //   }
  //   fclose(fo);
